...
def get_doc_dates(doc_path):
    try:
        word = win32com.client.Dispatch("Word.Application")
        doc = word.Documents.Open(doc_path)
        creation_date = format_doc_date(doc.BuiltInDocumentProperties("Creation Date").Value)
        modification_date = format_doc_date(doc.BuiltInDocumentProperties("Last Save Time").Value)
        doc.Close()
        word.Quit()
    except Exception as e:
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            ...
            if file.lower().endswith('.pdf'):
                ...
                creation_date, modification_date = get_doc_dates(file_path)
                file_type = 'Word'
